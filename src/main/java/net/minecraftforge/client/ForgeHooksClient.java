...
public class ForgeHooksClient {
    ...
    static final ThreadLocal<EnumWorldBlockLayer> renderLayer = ThreadLocal.withInitial(() -> EnumWorldBlockLayer.SOLID);
    ...
    public static void renderMainMenu(GuiMainMenu gui, FontRenderer font, int width, int height) {
        ...
        if (status == BETA || status == BETA_OUTDATED) {
            ...
            gui.drawString(font, line, (width - font.getStringWidth(line)) >> 1, 4, -1);
            line = I18n.format("forge.update.beta.2");
            gui.drawString(font, line, (width - font.getStringWidth(line)) >> 1, 4 + ((font.FONT_HEIGHT + 1)), -1);
        }
    ...
    public static void fillNormal(int[] faceData, EnumFacing facing) {
        Vector3f v1 = new Vector3f(faceData[21], faceData[22], faceData[23]);
        Vector3f t = new Vector3f(faceData[7], faceData[8], faceData[9]);
        Vector3f v2 = new Vector3f(faceData[14], faceData[15], faceData[16]);
        v1.sub(t);
        t.set(faceData[0], faceData[1], faceData[2]);
        v2.sub(t);
        v1.cross(v2, v1);
        v1.normalize();
