...
public class B3DModel {
    ...
    public static class Parser {
        ...
        private boolean isChunk(String tag) throws IOException {
            return Arrays.equals(this.tag, tag.getBytes(StandardCharsets.US_ASCII));
        }

        private void chunk(String tag) throws IOException {
            if (!isChunk(tag))
                throw new IOException("Expected chunk " + tag + ", got " + new String(this.tag, StandardCharsets.US_ASCII));
            pushLimit();
        }
        ...
        private B3DModel bb3d() throws IOException {
            ...
            if (version % 100 > Parser.version % 100)
                logger.warn("Minor version difference in model: " + ((float) version / 100));
                ...
                for (int i = 0; i < tex_coord_sets; i++) {
                    switch (tex_coord_set_size) {
                        ...
                        case 4:
                            ...
                            break;
                        default:
                            logger.error("Unsupported number of texture coords: " + tex_coord_set_size);
        ...
        private Map<Integer, Key> keys() throws IOException {
            ...
            while (buf.hasRemaining()) {
                ...
                animations.peek().put(frame, Optional.absent(), key);
                ret.put(frame, key);
            }
